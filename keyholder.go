...
// Keyholder manages keys and interactions on behalf of a user.
type Keyholder struct {
	...
	interactionCallback func(*Interaction)
...
// Connect connects to a peer for interaction history, imbalance information, and sending new interactions.
...
// SetInteractionCallback sets a callback to receive new interactions relevant to the keyholder.
func (w *Keyholder) SetInteractionCallback(callback func(*Interaction)) {
	w.interactionCallback = callback
}

// SetFilterPlotCallback sets a callback to receive new filter plots with confirmed interactions relevant to this keyholder.
func (w *Keyholder) SetFilterPlotCallback(callback func(*FilterPlotMessage)) {
	w.filterPlotCallback = callback
}

// GetGraph returns a public key's plot graph interactions as well as the corresponding plot height.
...
// GetRank returns a public key's interactivity rank as well as the corresponding plot height.
func (w *Keyholder) GetRank(pubKey ed25519.PublicKey) (float64, int64, error) {
	w.outChan <- Message{Type: "get_rank", Body: GetRankMessage{PublicKey: pubKey}}
	result := <-w.resultChan
	if len(result.err) != 0 {
...
// Send creates, signs and pushes an interaction out to the network.
func (w *Keyholder) Send(from, to ed25519.PublicKey, matures, expires int64, memo string) (
	...
	privKey, ok := decryptPrivateKey(encryptedPrivKey, w.passphrase)
	if !ok {
		return InteractionID{}, fmt.Errorf("Unable to decrypt private key")
	}
	...
	tx := NewInteraction(from, to, matures, expires, header.Height, memo)
	...
	// push it
	w.outChan <- Message{Type: "push_interaction", Body: PushInteractionMessage{Interaction: tx}}
	result := <-w.resultChan

	// handle result
	if len(result.err) != 0 {
		return InteractionID{}, fmt.Errorf("%s", result.err)
	}
	ptr := new(PushInteractionResultMessage)
	if err := json.Unmarshal(result.message, ptr); err != nil {
		return InteractionID{}, err
	}
	if len(ptr.Error) != 0 {
		return InteractionID{}, fmt.Errorf("%s", ptr.Error)
	}
	return ptr.InteractionID, nil
}

// GetInteraction retrieves information about a historic interaction.
func (w *Keyholder) GetInteraction(id InteractionID) (*Interaction, *PlotID, int64, error) {
	w.outChan <- Message{Type: "get_interaction", Body: GetInteractionMessage{InteractionID: id}}
	result := <-w.resultChan
	if len(result.err) != 0 {
		return nil, nil, 0, fmt.Errorf("%s", result.err)
	}
	t := new(InteractionMessage)
	if err := json.Unmarshal(result.message, t); err != nil {
		return nil, nil, 0, err
	}
	return t.Interaction, t.PlotID, t.Height, nil
}

// GetPublicKeyInteractions retrieves information about historic interactions involving the given public key.
func (w *Keyholder) GetPublicKeyInteractions(
	...
	gpkt := GetPublicKeyInteractionsMessage{
		PublicKey:   pubKey,
	...
	w.outChan <- Message{Type: "get_public_key_interactions", Body: gpkt}
	result := <-w.resultChan
	if len(result.err) != 0 {
		return 0, 0, 0, nil, fmt.Errorf("%s", result.err)
	}
	pkt := new(PublicKeyInteractionsMessage)
	...
	for {
		...
		case websocket.TextMessage:
			...
			case "push_interaction":
				pt := new(PushInteractionMessage)
				if err := json.Unmarshal(body, pt); err != nil {
					log.Printf("Error: %s, from: %s\n", err, w.conn.RemoteAddr())
					break
				}
				if w.interactionCallback != nil {
					w.interactionCallback(pt.Interaction)
				}
