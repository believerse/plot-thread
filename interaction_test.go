...
func TestInteraction(t *testing.T) {
	...
	// create the unsigned interaction
	tx := NewInteraction(pubKey, pubKey2, 0, 0, 0, "for lunch")
	...
	// re-sign the interaction with the wrong private key
	if err := tx.Sign(privKey2); err != nil {
		t.Fatal(err)
	}

	// verify the interaction (should fail)
	ok, err = tx.Verify()
	if err != nil {
		t.Fatal(err)
	}
...
func TestInteractionTestVector1(t *testing.T) {
	// create interaction for Test Vector 1
	...
	// re-sign the interaction with private key from test vector
