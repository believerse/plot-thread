...
func (b PlotStorageDisk) GetInteraction(id PlotID, index int) (
	*Interaction, *PlotHeader, error) {
	plotJson, err := b.GetPlotBytes(id)
	if err != nil {
		return nil, nil, err
	}

	// pick out and unmarshal the interaction at the index
	idx := "[" + strconv.Itoa(index) + "]"
	txJson, _, _, err := jsonparser.Get(plotJson, "interactions", idx)
	if err != nil {
		return nil, nil, err
	}
	tx := new(Interaction)
