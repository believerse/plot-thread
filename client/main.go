...
func main() {
	...
	prunePtr := flag.Bool("prune", false, "Prune interaction and public key interaction indices")
	...
	// 		if deviceCount != *numScribersPtr {
	// 			log.Fatalf("CUDA enabled but -numscribers is %d and supported devices is %d\n",
	// 				*numScribersPtr, deviceCount)
	// 		}
	// 		log.Println("CUDA initialized")
	// 	} else if OPENCL_ENABLED {
	// 		deviceCount := OpenCLInit()
	// 		if deviceCount != *numScribersPtr {
	// 			log.Fatalf("OpenCL enabled but -numscribers is %d and supported devices is %d\n",
	// 				*numScribersPtr, deviceCount)
	// 		}
	...
	// instantiate the interaction queue
	txQueue := NewInteractionQueueMemory(ledger)
