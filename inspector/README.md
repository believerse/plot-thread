...
2. `go install github.com/believerse/plot-thread/inspector`
...
* **tx** - Display the interaction specified with `-tx_id`.
* **history** - Display interaction history for the public key specified with `-pubkey`. Other options for this command include `-start_height`, `-end_height`, `-start_index`, and `-limit`.
* **verify** - Verify the sum of all public key imbalances matches what's expected dictated by the plot reward schedule. If `-pubkey` is specified, it verifies the public key's imbalance matches the imbalance computed using the public key's interaction history.
