...
func main() {
	...
	txIDPtr := flag.String("tx_id", "", "Interaction ID")
	startHeightPtr := flag.Int("start_height", 0, "Start plot height (for use with \"history\")")
	startIndexPtr := flag.Int("start_index", 0, "Start interaction index (for use with \"history\")")
	...
	var txID *InteractionID
	if len(*txIDPtr) != 0 {
		txIDBytes, err := hex.DecodeString(*txIDPtr)
		if err != nil {
			log.Fatal(err)
		}
		txID = new(InteractionID)
		copy(txID[:], txIDBytes)
	}
	...
	case "tx":
		...
		id, index, err := ledger.GetInteractionIndex(*txID)
		if err != nil {
			log.Fatal(err)
		}
		if id == nil {
			log.Fatalf("Interaction %s not found", *txID)
		}
		tx, header, err := plotStore.GetInteraction(*id, index)
		if err != nil {
			log.Fatal(err)
		}
		if tx == nil {
			log.Fatalf("No interaction found with ID %s\n", *txID)
		}
		displayInteraction(*txID, header, index, tx)

	case "history":
		if pubKey == nil {
			log.Fatal("-pubkey required for \"history\" command")
		}
		bIDs, indices, stopHeight, stopIndex, err := ledger.GetPublicKeyInteractionIndicesRange(
...
func displayPlot(id PlotID, plot *Plot) {
	b := concisePlot{
		ID:           id,
		Header:       *plot.Header,
		Interactions: make([]InteractionID, len(plot.Interactions)),
	}

	for i := 0; i < len(plot.Interactions); i++ {
		txID, err := plot.Interactions[i].ID()
		if err != nil {
			panic(err)
		}
		b.Interactions[i] = txID
	}
...
type txWithContext struct {
	...
	TxIndex     int           `json:"interaction_index_in_plot"`
	ID          InteractionID `json:"interaction_id"`
	Interaction *Interaction  `json:"interaction"`
}

func displayInteraction(txID InteractionID, header *PlotHeader, index int, tx *Interaction) {
...
type history struct {
	Interactions []txWithContext `json:"interactions"`
}

func displayHistory(bIDs []PlotID, indices []int, stopHeight int64, stopIndex int, plotStore PlotStorage) {
	h := history{Interactions: make([]txWithContext, len(indices))}
	for i := 0; i < len(indices); i++ {
		tx, header, err := plotStore.GetInteraction(bIDs[i], indices[i])
		if err != nil {
			panic(err)
		}
		if tx == nil {
			panic("No interaction found at index")
		}
		txID, err := tx.ID()
		if err != nil {
			panic(err)
		}
		h.Interactions[i] = txWithContext{
			PlotID:     bIDs[i],
			PlotHeader: *header,
			TxIndex:     indices[i],
			ID:          txID,
