...
func NewScriber(pubKeys []ed25519.PublicKey, memo string,
	plotStore PlotStorage, txQueue InteractionQueue,
...
func (m *Scriber) run() {
	...
	// register for new interactions
	newTxChan := make(chan NewTx, 1)
	m.processor.RegisterForNewInteractions(newTxChan)
	defer m.processor.UnregisterForNewInteractions(newTxChan)
	...
	for {
		...
		case newTx := <-newTxChan:
			log.Printf("Scriber %d received notice of new interaction %s\n", m.num, newTx.InteractionID)
			if plot == nil {
				// we're not working on a plot yet
				continue
			}

			if MAX_INTERACTIONS_TO_INCLUDE_PER_PLOT != 0 &&
				len(plot.Interactions) >= MAX_INTERACTIONS_TO_INCLUDE_PER_PLOT {
				log.Printf("Per-plot interaction limit hit (%d)\n", len(plot.Interactions))
				continue
			}

			// add the interaction to the plot (it updates the plotroot fee)
			if err := plot.AddInteraction(newTx.InteractionID, newTx.Interaction); err != nil {
				log.Printf("Error adding new interaction %s to plot: %s\n",
					newTx.InteractionID, err)
				// abandon the plot
				plot = nil
			}
...
func createNextPlot(tipID PlotID, tipHeader *PlotHeader, txQueue InteractionQueue,
	plotStore PlotStorage, ledger Ledger, pubKey ed25519.PublicKey, memo string) (*Plot, error) {

	// fetch interactions to confirm from the queue
	txs := txQueue.Get(MAX_INTERACTIONS_TO_INCLUDE_PER_PLOT - 1)
	...
	tx := NewInteraction(baseKey, pubKey, 0, 0, newHeight, memo)

	// prepend plotroot
	txs = append([]*Interaction{tx}, txs...)
