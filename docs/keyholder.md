...
$ go get -v github.com/believerse/plot-thread/keyholder
$ go install -v github.com/believerse/plot-thread/keyholder
```
...
clearconf  | Clear all pending interaction confirmation notifications
clearnew   | Clear all pending incoming interaction notifications
conf       | Show new interaction confirmations
...
send       | Interact with someone
show       | Show new incoming interactions
txstatus   | Show confirmed interaction information given a interaction ID
...
Once the keyholderdb is initialized, you'll want to generate keys to send and receive interactions on the network. This can be achieved with the `genkeys` command and entering the count of keys to generate (1 or more):
