...
type LedgerDisk struct {
	...
	prune      bool // prune historic interaction and public key interaction indices
}
...
// ConnectPlot connects a plot to the tip of the plot thread and applies the interactions to the ledger.
func (l LedgerDisk) ConnectPlot(id PlotID, plot *Plot) ([]InteractionID, error) {
	// sanity check
	...
	txIDs := make([]InteractionID, len(plot.Interactions))

	for i, tx := range plot.Interactions {
		txID, err := tx.ID()
		if err != nil {
			return nil, err
		}
		...
		// verify the interaction hasn't been processed already.
		// note that we can safely prune indices for interactions older than the previous series
		key, err := computeInteractionIndexKey(txID)
		if err != nil {
			return nil, err
		}
		...
		if ok {
			return nil, fmt.Errorf("Interaction %s already processed", txID)
		}

		// set the interaction index now
		indexBytes, err := encodeInteractionIndex(plot.Header.Height, i)
		if err != nil {
			return nil, err
		}
		...
		if tx.IsPlotroot() {
			...
			// during honest reorgs normal interactions usually get into the new most-work branch
			// but plotroots vanish. this mitigates the impact on UX when reorgs occur and interactions
			// depend on plotroots.
			txToApply = nil
			...
			if plot.Header.Height-PLOTROOT_MATURITY >= 0 {
				...
				oldTx, _, err := l.plotStore.GetInteraction(*oldID, 0)
				if err != nil {
					return nil, err
				}
				if oldTx == nil {
		...
		if txToApply != nil {
			...
			if !ok {
				txID, _ := txToApply.ID()
				return nil, fmt.Errorf("Sender has insufficient imbalance in interaction %s", txID)
			}
		}

		// associate this interaction with both parties
		if !tx.IsPlotroot() {
			key, err = computePubKeyInteractionIndexKey(tx.From, &plot.Header.Height, &i)
			if err != nil {
				return nil, err
			}
			batch.Put(key, []byte{0x1})
		}
		key, err = computePubKeyInteractionIndexKey(tx.To, &plot.Header.Height, &i)
		if err != nil {
			return nil, err
		}
		batch.Put(key, []byte{0x1})
	...
	// prune historic interaction and public key interaction indices now
...
// DisconnectPlot disconnects a plot from the tip of the plot thread and undoes the effects of the interactions on the ledger.
func (l LedgerDisk) DisconnectPlot(id PlotID, plot *Plot) ([]InteractionID, error) {
	// sanity check
	tipID, _, err := l.GetThreadTip()
	if err != nil {
		return nil, err
	...
	// disconnect interactions in reverse order
	for i := len(plot.Interactions) - 1; i >= 0; i-- {
		tx := plot.Interactions[i]
		txID, err := tx.ID()
		if err != nil {
			return nil, err
		}
		...
		// mark the interaction unprocessed now (delete its index)
		key, err := computeInteractionIndexKey(txID)
		if err != nil {
			return nil, err
		}
		...
		// unassociate this interaction with both parties
		if !tx.IsPlotroot() {
			key, err = computePubKeyInteractionIndexKey(tx.From, &plot.Header.Height, &i)
			if err != nil {
				return nil, err
...
// Prune interaction and public key interaction indices created by the plot at the given height
func (l LedgerDisk) pruneIndices(height int64, batch *leveldb.Batch) error {
	...
	for i, tx := range plot.Interactions {
		...
		// prune public key interaction indices
		if !tx.IsPlotroot() {
			key, err = computePubKeyInteractionIndexKey(tx.From, &plot.Header.Height, &i)
			if err != nil {
				return err
...
// Restore interaction and public key interaction indices created by the plot at the given height
func (l LedgerDisk) restoreIndices(height int64, batch *leveldb.Batch) error {
	...
	for i, tx := range plot.Interactions {
		...
		// restore public key interaction indices
		if !tx.IsPlotroot() {
			key, err = computePubKeyInteractionIndexKey(tx.From, &plot.Header.Height, &i)
			if err != nil {
				return err
...
// GetInteractionIndex returns the index of a processed interaction.
func (l LedgerDisk) GetInteractionIndex(id InteractionID) (*PlotID, int, error) {
	// compute the db key
	key, err := computeInteractionIndexKey(id)
	if err != nil {
		return nil, 0, err
	}
	...
	height, index, err := decodeInteractionIndex(indexBytes)
	if err != nil {
		return nil, 0, err
	}
...
// GetPublicKeyInteractionIndicesRange returns interaction indices involving a given public key
// over a range of heights. If startHeight > endHeight this iterates in reverse.
func (l LedgerDisk) GetPublicKeyInteractionIndicesRange(
	pubKey ed25519.PublicKey, startHeight, endHeight int64, startIndex, limit int) (
	[]PlotID, []int, int64, int, error) {

	if endHeight >= startHeight {
		// forward
		return l.getPublicKeyInteractionIndicesRangeForward(
			pubKey, startHeight, endHeight, startIndex, limit)
	}

	// reverse
	return l.getPublicKeyInteractionIndicesRangeReverse(
		pubKey, startHeight, endHeight, startIndex, limit)
}

// Iterate through interaction history going forward
func (l LedgerDisk) getPublicKeyInteractionIndicesRangeForward(
	pubKey ed25519.PublicKey, startHeight, endHeight int64, startIndex, limit int) (
	ids []PlotID, indices []int, lastHeight int64, lastIndex int, err error) {
	startKey, err := computePubKeyInteractionIndexKey(pubKey, &startHeight, &startIndex)
	if err != nil {
		return
	}

	endHeight += 1 // make it inclusive
	endKey, err := computePubKeyInteractionIndexKey(pubKey, &endHeight, nil)
	if err != nil {
		return
	}
	...
	for iter.Next() {
		_, lastHeight, lastIndex, err = decodePubKeyInteractionIndexKey(iter.Key())
		if err != nil {
			iter.Release()
...
// Iterate through interaction history in reverse
func (l LedgerDisk) getPublicKeyInteractionIndicesRangeReverse(
	pubKey ed25519.PublicKey, startHeight, endHeight int64, startIndex, limit int) (
	ids []PlotID, indices []int, lastHeight int64, lastIndex int, err error) {
	endKey, err := computePubKeyInteractionIndexKey(pubKey, &endHeight, nil)
	if err != nil {
...
func (l LedgerDisk) GetPublicKeyImbalanceAt(pubKey ed25519.PublicKey, height int64) (int64, error) {
	...
	startKey, err := computePubKeyInteractionIndexKey(pubKey, nil, nil)
	if err != nil {
		return 0, err
	}

	height += 1 // make it inclusive
	endKey, err := computePubKeyInteractionIndexKey(pubKey, &height, nil)
	if err != nil {
		return 0, err
	}
	...
	for iter.Next() {
		_, height, index, err := decodePubKeyInteractionIndexKey(iter.Key())
		if err != nil {
			iter.Release()
			return 0, err
		}
		...
		tx, _, err := l.plotStore.GetInteraction(*id, index)
		if err != nil {
			iter.Release()
			return 0, err
		}
		if tx == nil {
			iter.Release()
			return 0, fmt.Errorf("No interaction found in plot %s at index %d",
				*id, index)
		}
		...
		if bytes.Equal(pubKey, tx.To) {
			...
			iter.Release()
			txID, _ := tx.ID()
			return 0, fmt.Errorf("Interaction %s doesn't involve the public key", txID)
		}
	}
	iter.Release()
	if err := iter.Error(); err != nil {
...
const interactionIndexPrefix = 't'

const pubKeyInteractionIndexPrefix = 'k'
...
func computeInteractionIndexKey(id InteractionID) ([]byte, error) {
	key := new(bytes.Buffer)
	if err := key.WriteByte(interactionIndexPrefix); err != nil {
		return nil, err
	}
	if err := binary.Write(key, binary.BigEndian, id[:]); err != nil {
		return nil, err
...
func computePubKeyInteractionIndexKey(
	pubKey ed25519.PublicKey, height *int64, index *int) ([]byte, error) {
	key := new(bytes.Buffer)
	if err := key.WriteByte(pubKeyInteractionIndexPrefix); err != nil {
		return nil, err
	}
...
func decodePubKeyInteractionIndexKey(key []byte) (ed25519.PublicKey, int64, int, error) {
...
func encodeInteractionIndex(height int64, index int) ([]byte, error) {
	buf := new(bytes.Buffer)
	if err := binary.Write(buf, binary.BigEndian, height); err != nil {
		return nil, err
	}
...
func decodeInteractionIndex(indexBytes []byte) (int64, int, error) {
