...
import (
	...
	. "github.com/believerse/plot-thread"
...
func main() {
	...
	var newTxs []*Interaction
	var newConfs []*interactionWithHeight
	var newTxsLock, newConfsLock, cmdLock sync.Mutex

	// handle new incoming interactions
	keyholder.SetInteractionCallback(func(tx *Interaction) {
		ok, err := interactionIsRelevant(keyholder, tx)
		if err != nil {
			fmt.Printf("Error: %s\n", err)
			return
		}
	...
	keyholder.SetFilterPlotCallback(func(fb *FilterPlotMessage) {
		for _, tx := range fb.Interactions {
			ok, err := interactionIsRelevant(keyholder, tx)
			if err != nil {
				fmt.Printf("Error: %s\n", err)
				continue
			...
			newConfs = append(newConfs, &interactionWithHeight{tx: tx, height: fb.Header.Height})
			newConfsLock.Unlock()
			if showMessage {
				go func() {
					// don't interrupt a user during a command
	...
	completer := func(d prompt.Document) []prompt.Suggest {
		s := []prompt.Suggest{
			...
			{Text: "rank", Description: "Retrieve the current interactivity rank of all public keys"},
			...
			{Text: "show", Description: "Show new incoming interactions"},
			{Text: "txstatus", Description: "Show confirmed interaction information given a interaction ID"},
			{Text: "clearnew", Description: "Clear all pending incoming interaction notifications"},
			{Text: "conf", Description: "Show new interaction confirmations"},
			{Text: "clearconf", Description: "Clear all pending interaction confirmation notifications"},
	...
	for {
		...
		case "rank":
			...
			for i, pubKey := range pubKeys {
				rank, _, err := keyholder.GetRank(pubKey)
				if err != nil {
					fmt.Printf("Error: %s\n", err)
					break
				}
		...
		case "send":
			...
			id, err := sendInteraction(keyholder)
			if err != nil {
				fmt.Printf("Error: %s\n", err)
				break
			}
			fmt.Printf("Interaction %s sent\n", id)
		...
		case "txstatus":
			...
			txID, err := promptForInteractionID("ID", 2, bufio.NewReader(os.Stdin))
			if err != nil {
				fmt.Printf("Error: %s\n", err)
				break
			}
			fmt.Println("")
			tx, _, height, err := keyholder.GetInteraction(txID)
			if err != nil {
				fmt.Printf("Error: %s\n", err)
				break
			}
			if tx == nil {
				fmt.Printf("Interaction %s not found in the plotthread at this time.\n",
					txID)
				fmt.Println("It may be waiting for confirmation.")
				break
			}
			showInteraction(keyholder, tx, height)
		...
		case "show":
			...
			if tx != nil {
				showInteraction(keyholder, tx, 0)
				if left > 0 {
					fmt.Printf("\n%d new interaction(s) left to display. Type %s to continue.\n",
						left, aurora.Bold(aurora.Green("show")))
				}
			} else {
				fmt.Printf("No new interactions to display\n")
			}
		...
		case "conf":
			...
			tx, left := func() (*interactionWithHeight, int) {
				newConfsLock.Lock()
			...
			if tx != nil {
				showInteraction(keyholder, tx.tx, tx.height)
				if left > 0 {
		...
		gpkt:
			for i, pubKey := range pubKeys {
				...
				for {
					_, stopHeight, stopIndex, fbs, err := keyholder.GetPublicKeyInteractions(
...
// Prompt for interaction details and request the keyholder to send it
func sendInteraction(keyholder *Keyholder) (InteractionID, error) {
	...
	if len(memo) > MAX_MEMO_LENGTH {
		return InteractionID{}, fmt.Errorf("Maximum memo length (%d) exceeded (%d)",
			MAX_MEMO_LENGTH, len(memo))
	}

	// create and send send it. by default the interaction expires if not scribed within 3 plots from now
...
func promptForInteractionID(prompt string, rightJustify int, reader *bufio.Reader) (InteractionID, error) {
	fmt.Printf("%"+strconv.Itoa(rightJustify)+"v: ", aurora.Bold(prompt))
	text, err := reader.ReadString('\n')
	if err != nil {
		return InteractionID{}, err
	...
	text = strings.TrimSpace(text)
	if len(text) != 2*(len(InteractionID{})) {
		return InteractionID{}, fmt.Errorf("Invalid interaction ID")
	}
	idBytes, err := hex.DecodeString(text)
	if err != nil {
		return InteractionID{}, err
	}
	if len(idBytes) != len(InteractionID{}) {
		return InteractionID{}, fmt.Errorf("Invalid interaction ID")
	}
...
func showInteraction(w *Keyholder, tx *Interaction, height int64) {
...
func interactionIsRelevant(keyholder *Keyholder, tx *Interaction) (bool, error) {
	pubKeys, err := keyholder.GetKeys()
	if err != nil {
		return false, err
	}
...
type interactionWithHeight struct {
	tx     *Interaction
	height int64
}
