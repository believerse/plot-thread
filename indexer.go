...
func (idx *Indexer) run() {
	...
	for {
		...
		idx.indexInteractions(plot, *nextID, true)
	...
	for {
		...
		case tip := <-tipChangeChan:			
			...
			idx.indexInteractions(tip.Plot, tip.PlotID, tip.Connect)
			if !tip.More {
				idx.rankGraph()
			}
		case _, ok := <-idx.shutdownChan:
...
func (idx *Indexer) indexInteractions(plot *Plot, id PlotID, increment bool) {
	idx.latestPlotID = id
...
// Rank computes the InteractivityRank of every node in the directed graph.
	...
	for Δ > epsilon {
		...
		for source := range graph.nodes {
			for target, weight := range normalizedWeights[source] {
				graph.nodes[target].rank += alpha * nodes[source] * weight
			}

			graph.nodes[source].rank += (1-alpha)*inverse + leak*inverse
		}
