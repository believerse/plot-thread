...
// Processor processes plots and interactions in order to construct the ledger.
// It also manages the storage of all plot thread data as well as inclusion of new interactions into the interaction queue.
type Processor struct {
	genesisID               PlotID
	plotStore              PlotStorage                  // storage of raw plot data
	txQueue                 InteractionQueue              // queue of interactions to confirm
	ledger                  Ledger                        // ledger built from processing plots
	txChan                  chan txToProcess              // receive new interactions to process on this channel
	plotChan               chan plotToProcess           // receive new plots to process on this channel
	registerNewTxChan       chan chan<- NewTx             // receive registration requests for new interaction notifications
	unregisterNewTxChan     chan chan<- NewTx             // receive unregistration requests for new interaction notifications
	...
	newTxChannels           map[chan<- NewTx]struct{}     // channels needing notification of newly processed interactions
	tipChangeChannels       map[chan<- TipChange]struct{} // channels needing notification of changes to main thread tip plots
	shutdownChan            chan struct{}
	wg                      sync.WaitGroup
}

// NewTx is a message sent to registered new interaction channels when a interaction is queued.
type NewTx struct {
...
type txToProcess struct {
	...
	tx         *Interaction  // interaction to process
	source     string        // who sent it
...
func NewProcessor(genesisID PlotID, plotStore PlotStorage, txQueue InteractionQueue, ledger Ledger) *Processor {
	return &Processor{
		genesisID:               genesisID,
		plotStore:              plotStore,
		txQueue:                 txQueue,
...
// It verifies and processes plots and interactions.
...
func (p *Processor) run() {
	...
	for {
		...
		case txToProcess := <-p.txChan:
			// process a interaction
			err := p.processInteraction(txToProcess.id, txToProcess.tx, txToProcess.source)
			if err != nil {
				log.Println(err)
			}
		...
		case plotToProcess := <-p.plotChan:
			...
			log.Printf("Processing took %d ms, %d interaction(s), interaction queue length: %d\n",
				(after-before)/int64(time.Millisecond),
				len(plotToProcess.plot.Interactions),
				p.txQueue.Len())
...
// ProcessInteraction is called to process a new candidate interaction for the interaction queue.
func (p *Processor) ProcessInteraction(id InteractionID, tx *Interaction, from string) error {
...
// RegisterForNewInteractions is called to register to receive notifications of newly queued interactions.
func (p *Processor) RegisterForNewInteractions(ch chan<- NewTx) {
	p.registerNewTxChan <- ch
}

// UnregisterForNewInteractions is called to unregister to receive notifications of newly queued interactions
func (p *Processor) UnregisterForNewInteractions(ch chan<- NewTx) {
...
// Process a interaction
func (p *Processor) processInteraction(id InteractionID, tx *Interaction, source string) error {
	log.Printf("Processing interaction %s\n", id)

	// context-free checks
	if err := checkInteraction(id, tx); err != nil {
		return err
	}

	// no loose plotroots
	if tx.IsPlotroot() {
		return fmt.Errorf("Plotroot interaction %s only allowed in plot", id)
	}

	// is the queue full?
	if p.txQueue.Len() >= MAX_INTERACTION_QUEUE_LENGTH {
		return fmt.Errorf("No room for interaction %s, queue is full", id)
	}

	// is it confirmed already?
	plotID, _, err := p.ledger.GetInteractionIndex(id)
	if err != nil {
		return err
	}
	if plotID != nil {
		return fmt.Errorf("Interaction %s is already confirmed", id)
	}
	...
	if !checkInteractionSeries(tx, tipHeight+1) {
		return fmt.Errorf("Interaction %s would have invalid series", id)
	}

	// would it be mature if included in the next plot?
	if !tx.IsMature(tipHeight + 1) {
		return fmt.Errorf("Interaction %s would not be mature", id)
	}

	// is it expired if included in the next plot?
	if tx.IsExpired(tipHeight + 1) {
		return fmt.Errorf("Interaction %s is expired, height: %d, expires: %d",
	...
	// rejects a interaction if sender would have insufficient imbalance
	ok, err = p.txQueue.Add(id, tx)
	if err != nil {
		return err
	}
	if !ok {
		// don't notify others if the interaction already exists in the queue
		return nil
	}

	// notify channels
	for ch := range p.newTxChannels {
		ch <- NewTx{InteractionID: id, Interaction: tx, Source: source}
	}
	return nil
}

// Context-free interaction sanity checker
func checkInteraction(id InteractionID, tx *Interaction) error {
	// sane-ish time.
	// interaction timestamps are strictly for user and application usage.
	// we make no claims to their validity and rely on them for nothing.
	if tx.Time < 0 || tx.Time > MAX_NUMBER {
		return fmt.Errorf("Invalid interaction time, interaction: %s", id)
	}

	// no negative nonces
	if tx.Nonce < 0 {
		return fmt.Errorf("Negative nonce value, interaction: %s", id)
	}

	if tx.IsPlotroot() {
		// no maturity for plotroot
		if tx.Matures > 0 {
			return fmt.Errorf("Plotroot can't have a maturity, interaction: %s", id)
		}
		// no expiration for plotroot
		if tx.Expires > 0 {
			return fmt.Errorf("Plotroot can't expire, interaction: %s", id)
		}
		// no signature on plotroot
		if len(tx.Signature) != 0 {
			return fmt.Errorf("Plotroot can't have a signature, interaction: %s", id)
		}
	} else {
		// sanity check sender
		if len(tx.From) != ed25519.PublicKeySize {
			return fmt.Errorf("Invalid interaction sender, interaction: %s", id)
		}
		// sanity check signature
		if len(tx.Signature) != ed25519.SignatureSize {
			return fmt.Errorf("Invalid interaction signature, interaction: %s", id)
		}
	}

	// sanity check recipient
	if tx.To == nil {
		return fmt.Errorf("Interaction %s missing recipient", id)
	}
	if len(tx.To) != ed25519.PublicKeySize {
		return fmt.Errorf("Invalid interaction recipient, interaction: %s", id)
	}

	// no pays to self
	if bytes.Equal(tx.From, tx.To) {
		return fmt.Errorf("Interaction %s to self is invalid", id)
	}

	// make sure memo is valid ascii/utf8
	if !utf8.ValidString(tx.Memo) {
		return fmt.Errorf("Interaction %s memo contains invalid utf8 characters", id)
	}

	// check memo length
	if len(tx.Memo) > MAX_MEMO_LENGTH {
		return fmt.Errorf("Interaction %s memo length exceeded", id)
	}

	// sanity check maturity, expiration and series
	if tx.Matures < 0 || tx.Matures > MAX_NUMBER {
		return fmt.Errorf("Invalid maturity, interaction: %s", id)
	}
	if tx.Expires < 0 || tx.Expires > MAX_NUMBER {
		return fmt.Errorf("Invalid expiration, interaction: %s", id)
	}
	if tx.Series <= 0 || tx.Series > MAX_NUMBER {
		return fmt.Errorf("Invalid series, interaction: %s", id)
	}
...
func checkInteractionSeries(tx *Interaction, height int64) bool {	 
	if tx.IsPlotroot() {
		// plotroots must start a new series right on time
		return tx.Series == height/PLOTS_UNTIL_NEW_SERIES+1
	}

	// user interactions have a grace period (1 full series) to mitigate effects
...
func checkPlot(id PlotID, plot *Plot, now int64) error {
	...
	// sanity check interaction count
	if plot.Header.InteractionCount < 0 {
		return fmt.Errorf("Negative interaction count in header of plot %s", id)
	}

	if int(plot.Header.InteractionCount) != len(plot.Interactions) {
		return fmt.Errorf("Interaction count in header doesn't match plot %s", id)
	}

	// must have at least one interaction
	if len(plot.Interactions) == 0 {
		return fmt.Errorf("No interactions in plot %s", id)
	}

	// first tx must be a plotroot
	if !plot.Interactions[0].IsPlotroot() {
		return fmt.Errorf("First interaction is not a plotroot in plot %s", id)
	}

	// check max number of interactions
	max := computeMaxInteractionsPerPlot(plot.Header.Height)
	if len(plot.Interactions) > max {
		return fmt.Errorf("Plot %s contains too many interactions %d, max: %d",
			id, len(plot.Interactions), max)
	}

	// the rest must not be plotroots
	if len(plot.Interactions) > 1 {
		for i := 1; i < len(plot.Interactions); i++ {
			if plot.Interactions[i].IsPlotroot() {
				return fmt.Errorf("Multiple plotroot interactions in plot %s", id)
			}
		}
	}

	// basic interaction checks that don't depend on context
	txIDs := make(map[InteractionID]bool)
	for _, tx := range plot.Interactions {
		id, err := tx.ID()
		if err != nil {
			return err
		}
	...
	if len(txIDs) != len(plot.Interactions) {
		return fmt.Errorf("Duplicate interaction in plot %s", id)
	}

	// verify hash list root
	hashListRoot, err := computeHashListRoot(nil, plot.Interactions)
	if err != nil {
		return err
	}
...
// Computes the maximum number of interactions allowed in a plot at the given height. Inspired by BIP 101
func computeMaxInteractionsPerPlot(height int64) int {
	if height >= MAX_INTERACTIONS_PER_PLOT_EXCEEDED_AT_HEIGHT {
		// I guess we can revisit this sometime in the next 35 years if necessary
		return MAX_INTERACTIONS_PER_PLOT
	}

	// piecewise-linear-between-doublings growth
	doublings := height / PLOTS_UNTIL_INTERACTIONS_PER_PLOT_DOUBLING
	if doublings >= 64 {
		panic("Overflow uint64")
	}
	remainder := height % PLOTS_UNTIL_INTERACTIONS_PER_PLOT_DOUBLING
	factor := int64(1 << uint64(doublings))
	interpolate := (INITIAL_MAX_INTERACTIONS_PER_PLOT * factor * remainder) /
		PLOTS_UNTIL_INTERACTIONS_PER_PLOT_DOUBLING
	return int(INITIAL_MAX_INTERACTIONS_PER_PLOT*factor + interpolate)
}
...
func (p *Processor) acceptPlot(id PlotID, plot *Plot, now int64, source string) error {
	...
	for _, tx := range plot.Interactions {
		...
		if !checkInteractionSeries(tx, plot.Header.Height) {
			return fmt.Errorf("Interaction %s would have invalid series", txID)
		}
		if !tx.IsPlotroot() {
			if !tx.IsMature(plot.Header.Height) {
				return fmt.Errorf("Interaction %s is immature", txID)
			}
			if tx.IsExpired(plot.Header.Height) {
				return fmt.Errorf("Interaction %s is expired", txID)
			}
			...
			if !p.txQueue.ExistsSigned(txID, tx.Signature) {
				...
				if !ok {
					return fmt.Errorf("Signature verification failed, interaction: %s", txID)
				}
			}
		}
	}
...
// Update the ledger and interaction queue and notify undo tip channels
func (p *Processor) disconnectPlot(id PlotID, plot *Plot, source string) error {
	...
	// Add newly disconnected non-plotroot interactions back to the queue
	if err := p.txQueue.AddBatch(txIDs[1:], plot.Interactions[1:], plot.Header.Height-1); err != nil {
		return err
	}
...
// Update the ledger and interaction queue and notify new tip channels
func (p *Processor) connectPlot(id PlotID, plot *Plot, source string, more bool) error {
	...
	// Remove newly confirmed non-plotroot interactions from the queue
