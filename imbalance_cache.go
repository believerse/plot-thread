...
// It's used by Ledger when (dis-)connecting plots and by InteractionQueueMemory
// when deciding whether or not to add a interaction to the queue.
...
// Apply applies the effect of the interaction to the invovled parties' cached imbalances.
// It returns false if sender imbalance would go negative as a result of applying this interaction.
func (b *ImbalanceCache) Apply(tx *Interaction) (bool, error) {
	if !tx.IsPlotroot() {
...
// Undo undoes the effects of a interaction on the invovled parties' cached imbalances.
func (b *ImbalanceCache) Undo(tx *Interaction) error {
	if !tx.IsPlotroot() {
