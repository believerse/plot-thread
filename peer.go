...
func NewPeer(conn *websocket.Conn, genesisID PlotID, peerStore PeerStorage,
	...
	txQueue InteractionQueue, plotQueue *PlotQueue, addrChan chan<- string) *Peer {
	peer := &Peer{
		conn:                conn,
		genesisID:           genesisID,
		peerStore:           peerStore,
...
func (p *Peer) run() {
	...
	go func() {
		...
		// register to hear about new interactions
		newTxChan := make(chan NewTx, MAX_INTERACTIONS_TO_INCLUDE_PER_PLOT)
		p.processor.RegisterForNewInteractions(newTxChan)
		defer p.processor.UnregisterForNewInteractions(newTxChan)
		...
		for {
			...
			case tip := <-tipChangeChan:
				...
				log.Printf("Sending %s with %d interaction(s), to: %s\n",
					m.Type, len(fb.Interactions), p.conn.RemoteAddr())
				p.conn.SetWriteDeadline(time.Now().Add(writeWait))
				if err := p.conn.WriteJSON(m); err != nil {
					log.Printf("Write error: %s, to: %s\n", err, p.conn.RemoteAddr())
					p.conn.Close()
			...
			case newTx := <-newTxChan:
				...
				interested := func() bool {
					...
					return p.filterLookup(newTx.Interaction)
				}()
				if !interested {
					continue
				}

				// newly verified interaction announced, relay to peer
				pushTx := Message{
					Type: "push_interaction",
					Body: PushInteractionMessage{
						Interaction: newTx.Interaction,
					},
				}
				p.conn.SetWriteDeadline(time.Now().Add(writeWait))
			...
			case <-tickerUpdateWorkCheck.C:
				...
				txCount := len(p.workPlot.Interactions)
				if txCount == MAX_INTERACTIONS_TO_INCLUDE_PER_PLOT {
					// already at capacity
					break
				}
	...
	for {
		...
		case websocket.TextMessage:
			...
			case "get_rank":
				var gr GetRankMessage
				if err := json.Unmarshal(body, &gr); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					return
				}
				if err := p.onGetRank(gr.PublicKey, outChan); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					break
				}				
			...
			case "get_public_key_interactions":
				var gpkt GetPublicKeyInteractionsMessage
				if err := json.Unmarshal(body, &gpkt); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					return
				}
				if err := p.onGetPublicKeyInteractions(gpkt.PublicKey,
			...
			case "get_interaction":
				var gt GetInteractionMessage
				if err := json.Unmarshal(body, &gt); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					return
				}
				if err := p.onGetInteraction(gt.InteractionID, outChan); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					break
				}
			...
			case "push_interaction":
				var pt PushInteractionMessage
				if err := json.Unmarshal(body, &pt); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					return
				}
				if pt.Interaction == nil {
					log.Printf("Error: received nil interaction, from: %s\n", p.conn.RemoteAddr())
					return
				}
				if err := p.onPushInteraction(pt.Interaction, outChan); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					break
				}

			case "push_interaction_result":
				var ptr PushInteractionResultMessage
			...
			case "get_filter_interaction_queue":
				p.onGetFilterInteractionQueue(outChan)
...
// Handle a request for a public key's interactivity ranking
func (p *Peer) onGetRank(pubKey ed25519.PublicKey, outChan chan<- Message) error {
	log.Printf("Received get_rank from: %s\n", p.conn.RemoteAddr())
...
// Handle a request for a public key's interactions over a given height range
func (p *Peer) onGetPublicKeyInteractions(pubKey ed25519.PublicKey,
	startHeight, endHeight int64, startIndex, limit int, outChan chan<- Message) error {
	log.Printf("Received get_public_key_interactions from: %s\n", p.conn.RemoteAddr())

	if limit < 0 {
		outChan <- Message{Type: "public_key_interactions"}
		return nil
	}
	...
	// get the indices for all interactions for the given public key
	// over the given range of plot heights
	bIDs, indices, stopHeight, stopIndex, err := p.ledger.GetPublicKeyInteractionIndicesRange(
		pubKey, startHeight, endHeight, startIndex, limit)
	if err != nil {
		outChan <- Message{Type: "public_key_interactions", Body: PublicKeyInteractionsMessage{Error: err.Error()}}
		return err
	}
	...
	for i, plotID := range bIDs {
		// fetch interaction and header
		tx, plotHeader, err := p.plotStore.GetInteraction(plotID, indices[i])
		if err != nil {
			// odd case. just log it and continue
			log.Printf("Error retrieving interaction history, plot: %s, index: %d, error: %s\n",
		...
		if len(fbs) == 0 {
			...
			// interaction is from the same plot
			fb = fbs[len(fbs)-1]
		}
		fb.Interactions = append(fb.Interactions, tx)
	}

	// send it to the writer
	outChan <- Message{
		Type: "public_key_interactions",
		Body: PublicKeyInteractionsMessage{
			PublicKey:    pubKey,
...
// Handle a request for a interaction
func (p *Peer) onGetInteraction(txID InteractionID, outChan chan<- Message) error {
	log.Printf("Received get_interaction for %s, from: %s\n",
		txID, p.conn.RemoteAddr())

	plotID, index, err := p.ledger.GetInteractionIndex(txID)
	if err != nil {
		// not found
		outChan <- Message{Type: "interaction", Body: InteractionMessage{InteractionID: txID}}
		return err
	}
	if plotID == nil {
		// not found
		outChan <- Message{Type: "interaction", Body: InteractionMessage{InteractionID: txID}}
		return fmt.Errorf("Interaction %s not found", txID)
	}
	tx, header, err := p.plotStore.GetInteraction(*plotID, index)
	if err != nil {
		// odd case but send back what we know at least
		outChan <- Message{Type: "interaction", Body: InteractionMessage{PlotID: plotID, InteractionID: txID}}
		return err
	}
	if tx == nil {
		// another odd case
		...
		return fmt.Errorf("Interaction at plot %s, index %d not found",
			*plotID, index)
	}
...
// Handle receiving a interaction from a peer
func (p *Peer) onPushInteraction(tx *Interaction, outChan chan<- Message) error {
	id, err := tx.ID()
	if err != nil {
		outChan <- Message{Type: "push_interaction_result", Body: PushInteractionResultMessage{Error: err.Error()}}
		return err
	}

	log.Printf("Received push_interaction: %s, from: %s\n", id, p.conn.RemoteAddr())

	// process the interaction if this is the first time we've seen it
	var errStr string
	if !p.txQueue.Exists(id) {
		err = p.processor.ProcessInteraction(id, tx, p.conn.RemoteAddr().String())
		if err != nil {
			errStr = err.Error()
		}
	}

	outChan <- Message{Type: "push_interaction_result",
		Body: PushInteractionResultMessage{
			InteractionID: id,
			Error:         errStr,
		},
	}
...
// Handle a request to set a interaction filter for the connection
...
// Send back a filtered view of the interaction queue
func (p *Peer) onGetFilterInteractionQueue(outChan chan<- Message) {
	log.Printf("Received get_filter_interaction_queue, from: %s\n", p.conn.RemoteAddr())

	ftq := FilterInteractionQueueMessage{}
	...
	if p.filter == nil {
		...
		interactions := p.txQueue.Get(0)
		for _, tx := range interactions {
			if p.filterLookup(tx) {
				ftq.Interactions = append(ftq.Interactions, tx)
			}
		}
	}

	outChan <- Message{Type: "filter_interaction_queue", Body: ftq}
}

// Returns true if the interaction is of interest to the peer
func (p *Peer) filterLookup(tx *Interaction) bool {
	if p.filter == nil {
		return true
	}
...
func (p *Peer) createFilterPlot(id PlotID, plot *Plot) (*FilterPlotMessage, error) {
	...
	// filter out interactions the peer isn't interested in
...
func (p *Peer) updateReadLimit() {
	...
	// interactions are <500 bytes so this gives us significant wiggle room
	maxInteractions := computeMaxInteractionsPerPlot(height + 1)
	p.readLimit = int64(maxInteractions) * 1024
}
